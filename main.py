...
def get_args_parser():
    ...
    parser.add_argument('--reparam', type=str2bool, default=False)
    ...
    parser.add_argument('--eval_vizu', type=str2bool, default=False,
                        help='Perform evaluation only')
...
def plot_gradients(epoch, named_parameters):
    layers, ave_grads = get_gradients(named_parameters)
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(ave_grads, alpha=0.8, color="blue")
    ...
    # Sauvegarder la figure comme frame (image)
...
def main(args):
    ...
    if args.reparam and utils.is_main_process():
        model_with_reparam = model_without_ddp
        model_with_reparam.switch()
        n_parameters = sum(p.numel() for p in model_with_reparam.parameters() if p.requires_grad)
        print('number of params after re-param:', n_parameters)
    ...
    if args.eval:
        ...
        #TODO : utiliser un param : first eval
        ...
        print('[REAL] number of params after reparameterization:', n_parameters)
        print(f"[REAL] MAdds after reparameterization: {round(model_flops * 1e-6, 2)} M")

        test_stats = evaluate(data_loader_val, model, device, use_amp=args.use_amp)
        print(f"Accuracy of the network on {len(dataset_val)} test images: {test_stats['acc1']:.5f}%")
        
        if args.eval_vizu:
            test_stats = evaluate_vizualisation(data_loader_val, model, device, use_amp=args.use_amp)
        return
    ...
    if args.reparam and utils.is_main_process(): 
        total_params = count_parameters(model_with_reparam)
        # fvcore to calculate MAdds
        input_res = (3, args.input_size, args.input_size)
        input = torch.ones(()).new_empty((1, *input_res), dtype=next(model_with_reparam.parameters()).dtype,
                                        device=next(model_with_reparam.parameters()).device)
        flops = FlopCountAnalysis(model_with_reparam, input)
        model_flops = flops.total()
        print(f"Total Params After ReParam: {round(total_params * 1e-6, 2)} M")
        print(f"MAdds After ReParam: {round(model_flops * 1e-6, 2)} M")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        """if utils.is_main_process():
            if epoch%2 == 0:
                plot_gradients(epoch, model.named_parameters())"""
    ...
    """gif_path = "gradient_flow.gif"
    imageio.mimsave(gif_path, gradient_frames, fps=1)
    print(f"Saved gradient flow gif")"""
